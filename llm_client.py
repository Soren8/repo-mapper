...
def generate_summary(file_content, llm_endpoint, api_key=None, llm_model="deepseek/deepseek-chat"):
    ...
    prompt = f"Analyze the following file content and determine if it is a core code file (e.g., Python, JavaScript, etc.). If it is a core code file, provide a Markdown summary with sections: Key Functions, Complete Function List, Uses, and Used By. If it is not a core code file, provide a very concise summary (1-2 sentences) describing its purpose and relevance.\n\nFile Content:\n\n{file_content}"
